...
| # | Reported | State |
|---|---|---|
| 23 | ~~Storage runs out at about five seasons~~ **Re-measured 2026-09-16: no it does not** | The 834KB figure predates the storage work and is wrong by a factor of five. Measured now: **one seeded season is 161KB across 22 keys**, of which `db_players` is 88KB — and the registry is GLOBAL, so it is paid once rather than per season. Rolling over to a second season cost **9.6KB**, because a new season starts with empty boards and shares that registry. A fully worked season adds mainly its own board entries (~18KB each, ~54KB for three) plus charts and picks. So the ceiling is dozens of seasons, not three to five, and nobody will meet it by accident. The machinery to handle it exists anyway — see `utils/seasonStorage.js` — because a safety net nobody can trigger is a net nobody has tested; `addFiller()` is how it gets exercised. |
