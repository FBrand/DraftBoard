...
export function getScores(name) {
    ...
    const i = findMatchingIndex(name, buildNameIndex(players));
...
export function setScore(name, field, value) {
    if (!name || (field !== 'total' && field !== 'position')) return;
    const state = load();
    const i = findMatchingIndex(name, buildNameIndex(state.players));
    if (i === -1) state.players.push({ name, total: null, position: null, [field]: value });
